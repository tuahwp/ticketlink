...
import { createTicket, createEndCustomerSite, getEndCustomerSites } from "../actions";
...
export default function CreateTicketForm({
  ...
  const handleFormAction = async (formData: FormData) => {
    ...
    startTransition(async () => {
      try {
        // Inject values controlled by react state that aren't native input elements or need defaults
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans antialiased relative pb-16">
      ...
      <header className="relative border-b border-card-border bg-background/80 backdrop-blur-md sticky top-0 z-40 px-4 sm:px-6 lg:px-8 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-semibold uppercase tracking-wider">Support Center</span>
              <h1 className="text-base sm:text-lg font-bold text-foreground mt-0.5">Create Support Ticket</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
        <form action={handleFormAction} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-card border border-card-border rounded-2xl p-6 shadow-sm space-y-4">
                ...
                <div>
                  ...
                  {autoRefNo ? (
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-input-bg border border-card-border rounded-xl text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-555 text-sm font-mono"
                    />
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {selectedSiteId && (
                    <span className="text-[10px] text-emerald-605 dark:text-emerald-400 font-semibold block mt-1">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              >
                {isPending ? "Creating..." : "Save Ticket"}
              </button>
            </div>
          </div>
